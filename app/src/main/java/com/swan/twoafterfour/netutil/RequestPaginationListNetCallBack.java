...
/**
 * 网络请求回调，解析分页数组（与服务器约定数据格式）
 * Created by TongYuliang on 2017/10/19.
 */

public abstract class RequestPaginationListNetCallBack<T> extends RequestNetCallBack {

	@Override
	protected void onSuccessful(JSONObject jsonObject) throws JSONException {
		onSuccessful(parseArray(jsonObject.getJSONObject("Data").isNull("Records") ? null
				: jsonObject.getJSONObject("Data").getString("Records")));
	}
