...
import com.swan.twoafterfour.main.AddEventSetActivity;
import com.swan.twoafterfour.main.SelectEventSetAdapter;
...
public class SelectEventSetDialog extends Dialog implements View.OnClickListener,
	...
	public SelectEventSetDialog(Context context, OnSelectEventSetListener
			onSelectEventSetListener, int id) {
		...
		switch (v.getId()) {
			...
			case R.id.tvConfirm:
				if (mOnSelectEventSetListener != null) {
					mOnSelectEventSetListener.onSelectEventSet(mEventSets.get
							(mSelectEventSetAdapter.getSelectPosition()));
				}
				dismiss();
				break;
			case R.id.tvAddEventSet:
				((Activity) mContext).startActivityForResult(new Intent(mContext,
						AddEventSetActivity.class), ADD_EVENT_SET_CODE);
				break;
		}
	}
