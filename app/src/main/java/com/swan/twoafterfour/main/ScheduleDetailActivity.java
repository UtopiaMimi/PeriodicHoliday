...
import com.swan.twoafterfour.dialog.InputLocationDialog;
import com.swan.twoafterfour.dialog.SelectDateDialog;
import com.swan.twoafterfour.dialog.SelectEventSetDialog;
import com.swan.twoafterfour.task.eventset.LoadEventSetMapTask;
import com.swan.twoafterfour.task.schedule.UpdateScheduleTask;
import com.swan.twoafterfour.utils.DateUtils;
...
public class ScheduleDetailActivity extends BaseActivity implements View.OnClickListener,
		OnTaskFinishedListener<Map<Integer, EventSet>>, SelectDateDialog.OnSelectDateListener,
		InputLocationDialog.OnLocationBackListener, SelectEventSetDialog.OnSelectEventSetListener {
	...
	private void showSelectDateDialog() {
		if (mSelectDateDialog == null) {
			mSelectDateDialog = new SelectDateDialog(this, this, mSchedule.getYear(), mSchedule
					.getMonth(), mSchedule.getDay(), mPosition);
		}
	...
	private void setScheduleData() {
		...
		ivScheduleEventSetIcon.setImageResource(mSchedule.getEventSetId() == 0 ? R.mipmap
				.ic_detail_category : R.mipmap.ic_detail_icon);
		...
		if (requestCode == SelectEventSetDialog.ADD_EVENT_SET_CODE) {
			if (resultCode == AddEventSetActivity.ADD_EVENT_SET_FINISH) {
				...
				if (eventSet != null) {
					...
					sendBroadcast(new Intent(MainActivity.ADD_EVENT_SET_ACTION).putExtra
							(AddEventSetActivity.EVENT_SET_OBJ, eventSet));
				}
			}
		}
	}
	...
	private void resetDateTimeUi() {
		if (mSchedule.getTime() == 0) {
			if (mSchedule.getYear() != 0) {
				tvScheduleTime.setText(String.format(getString(R.string.date_format_no_time),
						mSchedule.getYear(), mSchedule.getMonth() + 1, mSchedule.getDay()));
			} else {
				tvScheduleTime.setText(R.string.click_here_select_date);
			}
		} else {
			tvScheduleTime.setText(DateUtils.timeStamp2Date(mSchedule.getTime(), getString(R
					.string.date_format)));
		}
	}
