...
import com.swan.twoafterfour.utils.JeekUtils;
import com.swan.twoafterfour.widget.StrikeThruTextView;
...
public class ScheduleAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
	...
	public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
		if (viewType == SCHEDULE_TYPE) {
			return new ScheduleViewHolder(LayoutInflater.from(mContext).inflate(R.layout
					.item_schedule, parent, false));
		} else if (viewType == SCHEDULE_FINISH_TYPE) {
			return new ScheduleFinishViewHolder(LayoutInflater.from(mContext).inflate(R.layout
					.item_schedule_finish, parent, false));
		} else if (viewType == SCHEDULE_CENTER) {
			return new ScheduleCenterViewHolder(LayoutInflater.from(mContext).inflate(R.layout
					.item_schedule_center, parent, false));
		} else {
			return new ScheduleBottomViewHolder(LayoutInflater.from(mContext).inflate(R.layout
					.item_schedule_bottom, parent, false));
		}
	}
	...
	public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
		if (holder instanceof ScheduleViewHolder) {
			...
			viewHolder.vScheduleHintBlock.setBackgroundResource(JeekUtils.getScheduleBlockView
					(schedule.getColor()));
			...
			if (schedule.getState() == 0) {
				...
				viewHolder.tvScheduleState.setTextColor(mContext.getResources().getColor(R.color
						.color_schedule_start));
			} else {
				...
				public void onClick(View v) {
					if (mBaseFragment instanceof ScheduleFragment) {
						mContext.startActivity(new Intent(mContext, ScheduleDetailActivity.class)
								...
								.putExtra(ScheduleDetailActivity.CALENDAR_POSITION, (
										(ScheduleFragment) mBaseFragment)
										.getCurrentCalendarPosition()));
					} else {
						mContext.startActivity(new Intent(mContext, ScheduleDetailActivity.class)
								.putExtra(ScheduleDetailActivity.SCHEDULE_OBJ, schedule)
			...
			if (mIsShowFinishTask) {
				RecyclerView.LayoutParams params = (RecyclerView.LayoutParams) viewHolder.itemView
						.getLayoutParams();
				params.height = LinearLayout.LayoutParams.WRAP_CONTENT;
				params.bottomMargin = mContext.getResources().getDimensionPixelSize(R.dimen
						.space_3dp);
			...
			viewHolder.tvChangeTaskList.setText(mIsShowFinishTask ? mContext.getString(R.string
					.schedule_hide_finish_task) : mContext.getString(R.string
					.schedule_show_finish_task));
			viewHolder.tvFinishHint.setVisibility(mIsShowFinishTask && mFinishSchedules.size() > 0
	...
	private void showDeleteScheduleDialog(final Schedule schedule) {
		new ConfirmDialog(mContext, R.string.schedule_delete_this_schedule, new ConfirmDialog
				.OnClickListener() {
			@Override
...
//								((ScheduleFragment) mBaseFragment).resetScheduleList();
	...
	protected class ScheduleViewHolder extends RecyclerView.ViewHolder {
		...
		public ScheduleViewHolder(View itemView) {
			...
			tvScheduleTitle = (TextView) itemView.findViewById(R.id.schedule_title_tv);
