...
import com.swan.twoafterfour.task.eventset.RemoveEventSetTask;
import com.swan.twoafterfour.utils.JeekUtils;
import com.swan.twoafterfour.widget.SlideDeleteView;
...
public class EventSetAdapter extends RecyclerView.Adapter<EventSetAdapter.EventSetViewHolder> {
	...
	public EventSetViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
		return new EventSetViewHolder(LayoutInflater.from(mContext).inflate(R.layout
				.item_event_set, parent, false));
	}
	...
	private void showDeleteEventSetDialog(final EventSet eventSet, final int position) {
		new ConfirmDialog(mContext, R.string.event_set_delete_this_event_set, new ConfirmDialog
				.OnClickListener() {
			@Override
