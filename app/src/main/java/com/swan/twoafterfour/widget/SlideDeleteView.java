package com.swan.twoafterfour.widget;
...
public class SlideDeleteView extends FrameLayout {
	...
	private void determineSpeed() {
		if ((moveX - downX) / (endTime - startTime) < -0.5) {
			mIsOpen = true;
			vDeleteView.startAnimation(new AutoMoveAnimation((long) (mWidth - mDeleteViewWidth / 2
					- vDeleteView.getX())));
	...
	private void determineTheState() {
		if (vDeleteView.getX() < mWidth - mDeleteViewWidth / 2) { // open
			...
			if (vDeleteView.getX() != mWidth - mDeleteViewWidth) {
				vDeleteView.startAnimation(new AutoMoveAnimation((long) (mWidth - mDeleteViewWidth
						/ 2 - vDeleteView.getX())));
			}
		} else { // close
			mIsOpen = false;
