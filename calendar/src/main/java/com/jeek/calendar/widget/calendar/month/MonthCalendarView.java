...
public class MonthCalendarView extends ViewPager implements OnMonthClickListener {
	...
	private void initAttrs(Context context, AttributeSet attrs) {
		initMonthAdapter(context, context.obtainStyledAttributes(attrs, R.styleable
				.MonthCalendarView));
	}
	...
	private OnPageChangeListener mOnPageChangeListener = new OnPageChangeListener() {
		...
		public void onPageSelected(final int position) {
			...
			if (monthView != null) {
				...
				if (mOnCalendarClickListener != null) {
					mOnCalendarClickListener.onPageChange(monthView.getSelectYear(), monthView
							.getSelectMonth(), monthView.getSelectDay());
				}
			} else {
