...
public class WeekCalendarView extends ViewPager implements OnWeekClickListener {
	...
	private void initAttrs(Context context, AttributeSet attrs) {
		initWeekAdapter(context, context.obtainStyledAttributes(attrs, R.styleable
				.WeekCalendarView));
	}
	...
	private OnPageChangeListener mOnPageChangeListener = new OnPageChangeListener() {
		...
		public void onPageSelected(final int position) {
			...
			if (weekView != null) {
				if (mOnCalendarClickListener != null) {
					mOnCalendarClickListener.onPageChange(weekView.getSelectYear(), weekView
							.getSelectMonth(), weekView.getSelectDay());
				}
