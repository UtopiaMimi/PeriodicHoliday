...
public class EventSetDao {
	...
	public Map<Integer, EventSet> getAllEventSetMap() {
		...
		Cursor cursor = db.query(JeekDBConfig.EVENT_SET_TABLE_NAME, null, null, null, null, null,
				null);
		EventSet eventSet;
	...
	public boolean removeEventSet(int id) {
		SQLiteDatabase db = mHelper.getWritableDatabase();
		int row = db.delete(JeekDBConfig.EVENT_SET_TABLE_NAME, String.format("%s=?", JeekDBConfig
				.EVENT_SET_ID), new String[]{String.valueOf(id)});
		db.close();
		mHelper.close();
		return row != 0;
	}
